...
import transfo.CopySrcToTrgSpace;
import blackboard.*;
...
//		blackboard = new CoherenceBlackboard();
	...
	public double createCopy() throws Exception{
		ITransformation copy = new CopySrcToTrgSpace(trgModelArea);
		double time = launch(copy, null, LinTraParameters.NUMBER_OF_THREADS_T1);
		return time;
	}
	
	public double launch(ITransformation transfo, ITransformation normalizeTransfo, int numThreads) throws Exception{
		
		double time1 = createCopy();
		
		double maxId = srcModelArea.size();
		
		IMaster master = new Master_SingleMT(workTODOArea, numThreads);
		((Master_SingleMT)master).organizeWork(srcModelArea);
		
		double time0 = System.currentTimeMillis();
		
		List<Thread> ts = new LinkedList<Thread>();
		for (int j=0; j<numThreads; j++){
			Thread t = new Thread(new Slave_SingleMT(j, transfo, workTODOArea));
			t.start();
			ts.add(t);
    	}
	...
	private double normalizeIds(ITransformation normalizationT, int numThreads) throws BlackboardException, InterruptedException {
		...
		IMaster master = new Master_SingleMT(workTODOArea, numThreads);
		((Master_SingleMT)master).organizeWork(trgModelArea);
		
		double time0 = System.currentTimeMillis();
		
		List<Thread> ts = new LinkedList<Thread>();
		for (int j=0; j<numThreads; j++){
			Thread t = new Thread(new Slave_SingleMT(j, normalizationT, workTODOArea));
			t.start();
			ts.add(t);
    	}
		for (int j=0; j<ts.size(); j++){
