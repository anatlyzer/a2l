...
public class DBLPv1_TestCase extends AbstractTestCase {
	...
	private static final String trafo = "DBLP_v1.atl";
	...
	public static void main(String[] args) throws Exception {
		...
		new DBLPv1_TestCase().doBenchmark(arguments, trafo);		
	}
	...
	protected Resource executeLintra(Resource input, int numThreads, boolean footprint, boolean optimised, AggregatedStatsRecorder recorder) throws Exception {
		...
		IFootprint footprintObject = footprint ? 
				new a2l.tests.dblp_v1.optimised.DBLPv1Footprint() : 
				null; 
		
		if ( optimised ) {
			outLintra = new a2l.tests.dblp_v1.optimised.DBLPv1Runner().
			...
			outLintra = new a2l.tests.dblp_v1.normal.DBLPv1Runner().
				setStatsRecorder(recorder).
				setFootprint(footprintObject).
				setIN(input).
				setOUT(OUT_MODEL_LINTRA, METAMODEL_AUTHOR_INFO).
