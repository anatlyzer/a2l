...
private Cache<Person, javaslang.collection.Set<Movie>> cache4_p2;
	private Cache<Person, javaslang.collection.Set<Movie>> cache1_p1;
	private Cache<Movie, java.util.List<Person>> cache2_m;
	private Cache<Person, java.util.List<Movie>> cache3_p1;public FindCouplesIMDbGlobalContext() {
cache4_p2 = com.google.common.cache.CacheBuilder.newBuilder().concurrencyLevel(Runtime.getRuntime().availableProcessors()).<Person, javaslang.collection.Set<Movie>>build();
...
cache2_m = com.google.common.cache.CacheBuilder.newBuilder().concurrencyLevel(Runtime.getRuntime().availableProcessors()).<Movie, java.util.List<Person>>build();
...
allInstancesCache = com.google.common.cache.CacheBuilder.newBuilder().concurrencyLevel(12).build();; }public javaslang.collection.Set<Movie> getcache4_p2(Person key, java.util.concurrent.Callable<javaslang.collection.Set<Movie>> closure) { try { return cache4_p2.get(key, closure); } catch (java.util.concurrent.ExecutionException e) { throw new IllegalStateException(); } }
public javaslang.collection.Set<Movie> getcache1_p1(Person key, java.util.concurrent.Callable<javaslang.collection.Set<Movie>> closure) { try { return cache1_p1.get(key, closure); } catch (java.util.concurrent.ExecutionException e) { throw new IllegalStateException(); } }
public java.util.List<Person> getcache2_m(Movie key, java.util.concurrent.Callable<java.util.List<Person>> closure) { try { return cache2_m.get(key, closure); } catch (java.util.concurrent.ExecutionException e) { throw new IllegalStateException(); } }
