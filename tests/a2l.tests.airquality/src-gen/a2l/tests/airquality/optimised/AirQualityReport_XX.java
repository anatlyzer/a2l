...
public class AirQualityReport_XX implements ITransformation, lintra2.transfo.ITransformation2 {
	...
	a2l.tests.airquality.optimised.AirQualityReportGlobalContext globalContext;
	a2l.runtime.GlobalTrace.PartialTrace trace = null;

	public AirQualityReport_XX(a2l.runtime.InputExtent inputExtent,
			a2l.tests.airquality.optimised.AirQualityReportGlobalContext global) {
	...
	final class PendingTask_Warning_date implements IPendingTask {
		...
		public PendingTask_Warning_date(airquality.Warning tgt, Object objId, a2l.runtime.IModel area,
		...
		public void execute(a2l.runtime.GlobalTrace globalTrace) {
			if (tgtElems != null && tgtElems.contains(objId)) {
				tgt.setDate((airquality.Date) getTargetResolveTempOrSame(objId, globalTrace));
			} else {
				tgt.setDate((airquality.Date) getTrace(objId, globalTrace));
			}
		}
	...
	public boolean check_highCO(java.lang.Object m1) throws BlackboardException {
		...
		boolean tmp112;
		boolean tmp113;
		java.util.List<AirMeasurement> r114;
		double get115;
		java.util.List<java.lang.Double> r116;
		double tmp117;
		boolean tmp118;
		int r119;
		int tmp120;
		boolean tmp121;
		boolean tmp122;
		if (m1 instanceof airquality.AirMeasurement) {
			...
			/*
			 * 27:25-27:242: MM!AirMeasurement.allInstances()->select(m2 | m1.station =
			 * m2.station and m1.date.year = m2.date.year and m1.date.month = m2.date.month
			 * and m1.date.day = m2.date.day)->collect(m | m.date.hour)->sortedBy(v |
			 * v)->first()
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			 * m1.date.day = m2.date.day)->collect(m | m.date.hour)->sortedBy(v | v)
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			 * m1.date.day = m2.date.day)->collect(m | m.date.hour)
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			...
			/*
			 * 28:27-28:264: MM!AirMeasurement.allInstances()->select(m2 | m1.station =
			 * m2.station and m1.date.year = m2.date.year and m1.date.month = m2.date.month
			 * and m1.date.day = m2.date.day and m2.date.hour < 1)->collect(m |
			 * m.date.minute)->sortedBy(v | v)->first()
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			 * m1.date.day = m2.date.day and m2.date.hour < 1)->collect(m |
			 * m.date.minute)->sortedBy(v | v)
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			 * m1.date.day = m2.date.day and m2.date.hour < 1)->collect(m | m.date.minute)
			 */
			/*
			 * null: MM!AirMeasurement.allInstances()->select(m2 | m1.station = m2.station
			 * and m1.date.year = m2.date.year and m1.date.month = m2.date.month and
			 * m1.date.day = m2.date.day and m2.date.hour < 1)
			 */
			...
			/*
			 * 31:10-34:39: MM!AirMeasurement.allInstances()->select(m2 | m1 <> m2 and
			 * m1.station = m2.station and m1.date.month = m2.date.month and m1.date.year =
			 * m2.date.year)->collect(m3 | m3.co)
			 */
			/*
			 * 31:10-33:50: MM!AirMeasurement.allInstances()->select(m2 | m1 <> m2 and
			 * m1.station = m2.station and m1.date.month = m2.date.month and m1.date.year =
			 * m2.date.year)
			 */
...
//			
	...
	public a2l.tests.airquality.optimised.AirQualityReport_XX setIN(IModel n) {
		INModel_ = n;

		return this;
	}

	public a2l.tests.airquality.optimised.AirQualityReport_XX setOUT(IOutputModel n) {
		OUTModel_ = n;
	...
	public void create_highCO(AirMeasurement m1) throws BlackboardException {
		...
		Date get15;
		int get16;
		w0 = airquality.AirqualityFactory.eINSTANCE.createWarning();
